...
- Moving a page is not a way to shed its provenance. `verify:guidance` reads all three published trees, so the `sources` declaration follows the page wherever it goes.
