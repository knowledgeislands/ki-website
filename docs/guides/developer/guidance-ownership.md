...
Carrying more upstream material makes provenance matter _more_, not less. Every page under `apps/site/src/guidance/` still declares the sources it was written from, and `verify:guidance` still gates that declaration. Nothing in the new default weakens it, and a page that carries more without declaring more is the failure mode this repository has always gated against.
